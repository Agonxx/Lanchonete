...
namespace Lanchonete.WEB.Utils
{
    public class HttpClientService : IDisposable
    {
        ...
        private readonly Dictionary<string, object> _bodyParams = new Dictionary<string, object>();
        ...
        public HttpClientService AddParamBody(string name, object value)
        {
            if (string.IsNullOrEmpty(name))
                throw new ArgumentNullException(nameof(name));

            _bodyParams[name] = value;
            return this;
        }
        ...
        private StringContent CriarConteudoHttp(object obj = null)
        {
            // Se temos parâmetros no body, usamos eles
            if (_bodyParams.Count > 0)
            {
                var jsonContent = JsonConvert.SerializeObject(_bodyParams);
                _logger?.LogDebug($"Conteúdo do body com parâmetros: {jsonContent}");
                _bodyParams.Clear();
                return new StringContent(jsonContent, Encoding.UTF8, "application/json");
            }

            // Caso contrário, usamos o objeto passado diretamente se existir
            if (obj != null)
            {
                var jsonContent = JsonConvert.SerializeObject(obj);
                _logger?.LogDebug($"Conteúdo do body com objeto: {jsonContent}");
                return new StringContent(jsonContent, Encoding.UTF8, "application/json");
            }
        ...
        // Métodos POST sem objeto obrigatório
        public async Task<ApiResponse<T>> PostAsync<T>(string controller, CancellationToken cancellationToken = default, params object[] parametros)
        {
            try
            {
                var response = await PostAsyncInternal(null, controller, cancellationToken, parametros);
                return await ProcessarResposta<T>(response);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ApiResponse<T>> PostAsync<T>(object obj, string controller, CancellationToken cancellationToken = default, params object[] parametros)
        {
            try
            {
                var response = await PostAsyncInternal(obj, controller, cancellationToken, parametros);
                return await ProcessarResposta<T>(response);
            }
            catch (Exception ex)
            {
        ...
        private async Task<HttpResponseMessage> PostAsyncInternal(object obj, string controller, CancellationToken cancellationToken = default, params object[] parametros)
        {
            MontarUrl(controller, parametros);
            var content = CriarConteudoHttp(obj);
            var response = await _client.PostAsync(_url, content, cancellationToken);
            return response;
        }

        // Métodos PUT sem objeto obrigatório
        public async Task<ApiResponse<T>> PutAsync<T>(string controller, CancellationToken cancellationToken = default, params object[] parametros)
        {
            try
            {
                var response = await PutAsyncInternal(null, controller, cancellationToken, parametros);
                return await ProcessarResposta<T>(response);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ApiResponse<T>> PutAsync<T>(string controller, object obj, CancellationToken cancellationToken = default, params object[] parametros)
        {
            try
            {
                var response = await PutAsyncInternal(obj, controller, cancellationToken, parametros);
                return await ProcessarResposta<T>(response);
            }
            catch (Exception ex)
            {
        ...
        private async Task<HttpResponseMessage> PutAsyncInternal(object obj, string controller, CancellationToken cancellationToken = default, params object[] parametros)
        {
            MontarUrl(controller, parametros);
            var content = CriarConteudoHttp(obj);
            var response = await _client.PutAsync(_url, content, cancellationToken);
            return response;
        }
